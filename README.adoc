...
* *rwt*: A commit that rewrite some part of code, *similar* to `refactor`.
...
rwt,buffer: reduce the write_buf() complexity to O(n)
fix,io/net: fix CVE-2021-9999
doc,readme: add guideline for using docker build

# breaking change
rwt!lib/crypto: drop the SHA-1 library support
...
Use the `rwt` type when you modify existing code, whatever it contains
